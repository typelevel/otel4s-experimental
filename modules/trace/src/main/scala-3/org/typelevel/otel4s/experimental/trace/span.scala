...
/** Wraps the body of an annotated method or variable into the span.
  *
  * By default, the span name will be `className.methodName`, unless a name is provided as an
  * argument.
  *
  * {{{
  ...
  *   @span
  *   def findUser(@attribute userId: Long): F[User] =
  *     db.findUser(id)
  * }
  *
  *  // expands into
  ...
  *
  * @note
  *   macro remains experimental in Scala 3. Therefore, the enclosing class must be annotated with
  *   `@scala.annotation.experimental`.
  *
  * @param name
  *   the custom name of the span. If not specified, the span name will be `className.methodName`
  *
  * @param debug
...
class span(
  ...
  override def transform(using
    ...
    def abort(message: String) =
      report.errorAndAbort(s"@span macro: $message", tree.pos)
    ...
    val (nameParam, debug) = {
      ...
      tree.symbol.getAnnotation(TypeRepr.of[span].typeSymbol) match {
        ...
        case Some(other) =>
          abort(s"unknown structure of the @span annotation: $other.")

        case None =>
          abort("the @span annotation is missing.")
      }
    }
    ...
    def expandDef(defDef: DefDef): quotes.reflect.Definition = {
      ...
      val attributes: List[Expr[Attribute[_]]] =
        params.flatMap { case vd @ ValDef(name, tpt, _) =>
          ...
          sym.getAnnotation(TypeRepr.of[attribute].typeSymbol) match {
    ...
    if (debug) {
      ...
      scala.Predef.println(s"@span $at- expanded into:\n${result.show}")
    }
