...
class span(
  ...
  def macroTransform(annottees: Any*): Any = macro spanMacro.impl
}
...
object spanMacro {
  ...
  def impl(c: blackbox.Context)(annottees: c.Expr[Any]*): c.Expr[c.Tree] = {
    ...
    def abort(message: String) =
      c.abort(c.enclosingPosition, s"@span macro: $message")
      ...
      val attributes = defDef.vparamss.flatten.flatMap {
        case ValDef(mods, name, tpt, _) =>
          mods.annotations.flatMap { annotation =>
            ...
            if (
              typed.tpe.typeSymbol.fullName == "org.typelevel.otel4s.experimental.trace.attribute"
            ) {
              val keyArg = typed match {
                case q"new ${_}(${keyArg})" =>
                  keyArg
                case _ =>
                  abort("unknown structure of the @attribute annotation.")
              }
